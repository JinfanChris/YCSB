...
public final class Client {
   ...
   * among multiple servers if the client is the bottleneck. Additionally
  ...
  public static void usageMessage() {
    ...
    System.out.println(
        "To run the load phase from multiple servers, start a separate client on each;"
            + " additionally,");
    System.out.println(
  ...
  private static void exportMeasurements(Properties props, int opcount, long runtime)
    ...
    try {
      ...
      String exporterStr = props.getProperty(
          EXPORTER_PROPERTY, "site.ycsb.measurements.exporter.TextMeasurementsExporter");
      try {
  ...
  public static void main(
    ...
    Runtime.getRuntime().addShutdownHook(
            new Thread(() -> {
          try {
            System.err.println("Shutdown signal received. Dumping final YCSB stats...");
            ...
            for (Map.Entry<Thread, ClientThread> entry : threads.entrySet()) {
              try {
                entry.getKey().join();
                gopsDone += entry.getValue().getOpsDone();
                System.out.println("finished with "
                        + entry.getValue().getOpsDone()
                        + " operations.");
              } catch (InterruptedException ignored) {
                System.out.println("Thread interrupted while joining. Ignoring." + ignored);
              }
            }

            if (gprops != null && gstartTime > 0) {
              long runtime = now - gstartTime;
              exportMeasurements(gprops, gopsDone, runtime);
            } else {
              System.err.println(
                  "Stats not available yet. Benchmark may not have started properly.");
            }
          } catch (IOException e) {
            e.printStackTrace();
          }
  ...
  private static List<ClientThread> initDb(
      String dbname,
      Properties props,
      int threadcount,
    ...
    try (final TraceScope span = tracer.newScope(CLIENT_INIT_SPAN)) {
      ...
      if (threadcount > opcount && opcount > 0) {
        threadcount = opcount;
        System.out.println(
            "Warning: the threadcount is bigger than recordcount, the threadcount will be"
                + " recordcount!");
      }
  ...
  private static void initWorkload(
      Properties props, Thread warningthread, Workload workload, Tracer tracer) {
    try {
      ...
      if (argindex < args.length) {
        System.out.println(
            "An argument value without corresponding argument specifier (e.g., -p, -s) was found. "
                + "We expected an argument specifier and instead found "
                + args[argindex]);
      } else {
        System.out.println(
